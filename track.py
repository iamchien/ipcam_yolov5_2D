...
rtsp = 'rtsp://admin:chien197@192.168.1.123:554/onvif1'
# img_path = '/home/hongthinh/haison98/ChienUET/5fps/frame3.jpg'
txt_dir = './inference/txt_output'

from api import Robotic_API
api = Robotic_API()
api.init_server()

def detect(opt, txtDir = txt_dir):
    out, evaluate = opt.output, opt.evaluate
    # Initialize
    ...
    if not evaluate:
        ...
        if os.path.exists(txtDir):
            shutil.rmtree(txtDir)  # delete output folder
        os.makedirs(txtDir)  # make new output folder
    # Load model
...
def run(opt, model, device, source=rtsp):
    out, deep_sort_weights, show_vid, save_vid, save_txt, imgsz, half = \
        opt.output, opt.deep_sort_weights, opt.show_vid, opt.save_vid, \
            opt.save_txt, opt.imgsz, opt.half
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)): 
                        ...
                        if save_txt:
                            ...
                            idx = frame_idx + 1
                            txt_path = 'output.txt'
                            txt_path = os.path.join(txt_dir,txt_path)
                            with open(txt_path, 'a') as f:
                               f.write(('%g ' * 8 + '\n') % (idx, len(outputs), c, id, bbox_left,
...
def parse_opt():
    ...
    parser.add_argument('--yolo_weights', nargs='+', type=str, default='./yolov5/weights/yolov5x6.pt', help='model.pt path(s)')
    parser.add_argument('--deep_sort_weights', type=str, default='deep_sort_pytorch/deep_sort/deep/checkpoint/ckpt.t7', help='ckpt.t7 path')
    # file/folder, 0 for webcam
    #parser.add_argument('--source', type=str, default='0', help='source')
    parser.add_argument('--output', type=str, default='./inference/video_output', help='output folder')  # output folder
